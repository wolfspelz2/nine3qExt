...
namespace nine3q.Items.Test
{
    ...
    public class InventoryTest
    {
        ...
        public void CreateItem_dont_re_use_id()
        {
            // Arrange
            var inv = new Inventory();
            inv.CreateItem(new PropertySet { [Pid.Id] = 1L });
            inv.CreateItem(new PropertySet { [Pid.Id] = 2L });
            inv.CreateItem(new PropertySet { [Pid.Id] = 3L });
            inv.DeleteItem(2);

            // Act
            var nextId = inv.CreateItem(new PropertySet { }).Id;

            // Assert
            Assert.AreNotEqual(2, nextId);
        }
