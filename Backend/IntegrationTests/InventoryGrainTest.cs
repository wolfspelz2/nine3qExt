...
namespace IntegrationTests
{
    ...
    public class InventoryGrainTest
    {
        ...
        /*
            ...
            {
                // Arrange
                var invName = "Test-" + Utils.GetMethodName(StackFrameNumber) + "-" + RandomString.Get(10);
                var inv = GrainClient.GrainFactory.GetGrain<IInventory>("Test-" + Utils.GetMethodName(StackFrameNumber) + "-" + RandomString.Get(10));
                try {
                    var props = new PropertySet {
                    { Pid.TestInt, (long)41 },
            ...
            {
                // Arrange
                var uniqueName = "Test-" + Utils.GetMethodName(StackFrameNumber) + "-" + RandomString.Get(10);
            ...
            {
                // Arrange
                var source = GrainClient.GrainFactory.GetGrain<IInventory>("Test-" + Utils.GetMethodName(StackFrameNumber) + "-" + RandomString.Get(10) + "-source");
                var dest = GrainClient.GrainFactory.GetGrain<IInventory>("Test-" + Utils.GetMethodName(StackFrameNumber) + "-" + RandomString.Get(10) + "-dest");
                try {
                    // Arrange
                    ...
                    source.DeletePermanentStorage().Wait();
                    dest.DeletePermanentStorage().Wait();
                }
            }
            ...
            public void TransferItem_with_weird_properties()
            {
                ...
                try {
                    ...
                    var itemId = source.CreateItem(new PropertySet {
                        ...
                        [Pid.TestItem] = new ItemId((int)42),
                    ...
                    Assert.AreEqual(new ItemId(42).ToString(), destProps[map[itemId]].GetItem(Pid.TestItem).ToString());
                    Assert.AreEqual(new ItemId(42).ToString(), destProps[map[itemId]].GetItem(Pid.TestItem2).ToString());
                    Assert.AreEqual(new ItemId(42).ToString(), destProps[map[itemId]].GetItem(Pid.TestItem3).ToString());
            ...
            public void CancelItemTransfer_deletes_received_items_and_resets_source()
            {
                ...
                try {
                    ...
                    Assert.AreEqual(3, new ItemIdSet(source.GetItems().Result).Count);
                    Assert.AreEqual(0, new ItemIdSet(dest.GetItems().Result).Count);
            ...
            public void transient_inventory_persist_completely_on_WritePermanentStorage()
            {
                ...
                try {
                    ...
                    var itemId1 = inv.CreateItem(new PropertySet { { Pid.TestInt, (long)41 } }).Result;
                    var itemId2 = inv.CreateItem(new PropertySet { { Pid.TestInt, (long)42 } }).Result;
                    inv.WritePermanentStorage().Wait();
                    inv.Deactivate().Wait();
