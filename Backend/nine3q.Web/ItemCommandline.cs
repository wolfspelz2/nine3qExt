...
namespace nine3q.Web
{
    public class ItemCommandline: Commandline, ICommandline
    {
        ...
        public ItemCommandline(string path): base(path)
        {
            ...
            Handlers.Add(Fn.Admin_Environment.ToString(), new Handler { Name = Fn.Admin_Environment.ToString(), Function = Admin_Environment, Role = Role.Public.ToString(), Description = "Show environment variables", });
            Handlers.Add(Fn.Admin_Process.ToString(), new Handler { Name = Fn.Admin_Process.ToString(), Function = Admin_Process, Role = Role.Public.ToString(), Description = "Show process info", });
        ...
        object Admin_CreateRole(Arglist args)
        {
            ...
            OrleansInitializer.Initialize();
            var tmpl = GrainClient.GrainFactory.GetGrain<IInventory>(InventoryService.TemplatesInventoryName);
            var tmplId = tmpl.GetItemByName(tmplName).Result;
            if (tmplId == ItemId.NoItem) { return $"Missing template={tmplName}"; }
            var inv = GrainClient.GrainFactory.GetGrain<IInventory>(userName);
            var user = GrainClient.GrainFactory.GetGrain<IUser>(userName);
            user.Prepare().Wait();
        ...
        class ItemReference : IComparable
        {
            ...
            public ItemId Item { get; protected set; }

            public ItemReference(string inventoryName, ItemId itemId)
            {
        ...
        {
            return new ItemReference("Inventory", new ItemId(args.Get("ID")));
        }
        ...
        object Inventory_DeleteItem(Arglist args)
        {
            args.Next("cmd");
            var inventoryName = args.Next("Inventory");
            var itemId = new ItemId(args.Next("item ID"));
            OrleansInitializer.Initialize();
            var inv = GrainClient.GrainFactory.GetGrain<IInventory>(inventoryName);
            var ids = new ItemIdSet(inv.GetItems().Result);
            var deleted = inv.DeleteItem(itemId).Result;
            var newIds = new ItemIdSet(inv.GetItems().Result);
        ...
        string Inventory_GetItemProperties_FormatValue(string inventoryName, Pid pid, object value)
        {
            ...
            switch (Property.Get(pid).Type) {
                case Property.Type.Item:
                    s = ShowItemLink(inventoryName, value as ItemId);
                    break;
        ...
        object Inventory_GetItemProperties(Arglist args)
        {
            ...
            if (!string.IsNullOrEmpty(templateName)) {
                try {
                    var templateInv = GrainClient.GrainFactory.GetGrain<IInventory>(GrainInterfaces.InventoryService.TemplatesInventoryName);
            ...
            if (format == Inventory_Result_format.table.ToString()) {
                ...
                foreach (var pair in props) {
                    JsonTree.Node propNode = null;
                    switch (Property.Get(pair.Key).Type) {
                        case Property.Type.Int:
                            propNode = new JsonTree.Node(JsonTree.Node.Type.Int, (long)pair.Value);
                            break;
                        case Property.Type.Float:
                            propNode = new JsonTree.Node(JsonTree.Node.Type.Float, (double)pair.Value);
                            break;
                        case Property.Type.Bool:
                            propNode = new JsonTree.Node(JsonTree.Node.Type.Bool, (bool)pair.Value);
                            break;
                        case Property.Type.Item:
                            propNode = new JsonTree.Node(JsonTree.Node.Type.Int, (long)(ItemId)pair.Value);
                            break;
                        default:
                            propNode = new JsonTree.Node(JsonTree.Node.Type.String, pair.Value.ToString());
                            break;
                    }
        ...
        object Inventory_AddChildToContainer(Arglist args)
        {
            ...
            var inventoryName = args.Next("Inventory");
            var itemId = new ItemId(args.Next("item ID"));
            var containerId = new ItemId(args.Next("Container ID"));
            OrleansInitializer.Initialize();
            var inv = GrainClient.GrainFactory.GetGrain<IInventory>(inventoryName);
        ...
        object Inventory_TransferItem(Arglist args)
        {
            ...
            var source = GrainClient.GrainFactory.GetGrain<IInventory>(sourceInventory);
            var dest = GrainClient.GrainFactory.GetGrain<IInventory>(destinationInventory);
        ...
        PropertySet GetPropertySetFromNextArgs(Arglist args)
        {
            ...
            do {
                ...
                if (arg != "") {
                    if (arg.StartsWith("{")) {
                        var jsonNode = new JsonTree.Node(arg);
        ...
        string ShowItemLink(string inventoryName, ItemId itemId)
        {
            var inv = GrainClient.GrainFactory.GetGrain<IInventory>(inventoryName);
        ...
        private object Translation_Set(Arglist args)
        {
            ...
            GrainClient.GrainFactory.GetGrain<ICachedString>(cacheKey).Set(translated, CachedStringOptions.Infinite, CachedStringOptions.Persistent).Wait();
            return GrainClient.GrainFactory.GetGrain<ICachedString>(cacheKey).Get().Result;
        }
        ...
        private object Translation_Get(Arglist args)
        {
            ...
            OrleansInitializer.Initialize();
            var cacheKey = GrainInterfaces.Content.GetTranslationCacheKey(key, lang);
            var translated = GrainClient.GrainFactory.GetGrain<ICachedString>(cacheKey).Get().Result;
            return translated;
        }
        ...
        private object Translation_Unset(Arglist args)
        {
            ...
            OrleansInitializer.Initialize();
            var cacheKey = GrainInterfaces.Content.GetTranslationCacheKey(key, lang);
            GrainClient.GrainFactory.GetGrain<ICachedString>(cacheKey).Unset().Wait();
            return "(ok)";
        }
        ...
        object Content_Groups(Arglist args)
        {
            ...
            var inv = GrainClient.GrainFactory.GetGrain<IContentGenerator>(inventoryName);
        ...
        private object Content_CreateTemplates(Arglist args)
        {
            ...
            OrleansInitializer.Initialize();
            var inventoryName = GrainInterfaces.InventoryService.TemplatesInventoryName;
            var ids = new ItemIdSet(GrainClient.GrainFactory.GetGrain<IContentGenerator>(inventoryName).CreateTemplates(name).Result);
        ...
        object Room_Statistics(Arglist args)
        {
            ...
            OrleansInitializer.Initialize();
            var user = GrainClient.GrainFactory.GetGrain<IRoom>(userName);
            var stats = user.GetStatistics().Result;
            var table = new Table();
            foreach (var pair in stats) {
                table.Grid.Add(new Table.Row() {
        ...
        object Room_ReceiveFromUser(Arglist args)
        {
            ...
            var data = args.Next("Data");
            OrleansInitializer.Initialize();
            var room = GrainClient.GrainFactory.GetGrain<IRoom>(roomName);
